...
# Table of Contents
- [Purposes](./README.md#purposes)
- [How to use this](./README.md#how-to-use-this)
...
- if `touch Exists && git add Exists`: `` @`Exists` `` or `` ?`Exists` ``.
  ...
  - `*` is not used as update prefix, since `*` has much other uses: for _Regular Expressions_ (wildcards), plus for _C++_ (such as block co
mments, dereferences, or math).
...
### Affiliates
You can use [_Capital 1_'s affiliate program](https://www.capitalone.com/bank/referrals/checking300/friend/?referralCode=06BEGSDPR&programCode=RAFOFFER300) to support `SusuMid` (the revenue allows to publish more for you).
